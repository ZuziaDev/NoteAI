    ...
    <section className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {NOTE_TEMPLATES.map((template) => (
            <button
              ...
              disabled={isBusy !== null || isSaving}
              className="rounded-xl border border-cyan-200/25 bg-cyan-300/15 px-3 py-2 text-sm text-cyan-100 transition hover:bg-cyan-300/25 disabled:cursor-not-allowed disabled:opacity-60"
            >
      ...
      <div className="rounded-2xl border border-white/10 bg-black/25 p-3">
        <div className="flex gap-2 overflow-x-auto pb-1">
          {visibleNotes.length === 0 ? (
            <span className="px-2 py-1 text-xs text-slate-400">{t('Sekme yok', 'No tabs')}</span>
          ) : (
            visibleNotes.map((note) => {
              ...
              return (
                <button
                  ...
                  className={`min-w-max rounded-xl border px-3 py-2 text-xs transition ${
                    selected
                      ? 'accent-soft'
      ...
      <div className="grid gap-3 xl:grid-cols-2">
          ...
          {activeMeta ? (
            <>
              <textarea
                ...
                className="mt-3 h-[360px] w-full resize-none rounded-xl border border-white/10 bg-black/20 p-3 text-sm leading-relaxed text-slate-100 outline-none placeholder:text-slate-500"
              />
              <div className="mt-2 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-400">
                <span>
                  {activeMeta.kind === 'external' ? t('Dis Dosya', 'External File') : 'Local'} •{' '}
                  {activeMeta.fileName}
                </span>
                <span>{formatDateTime(activeMeta.updatedAt, locale)}</span>
              </div>
              <p className="mt-1 break-all text-xs text-slate-500">{activeMeta.path}</p>
              <button
                onClick={() => void handleDeleteNote()}
                disabled={isBusy !== null || isSaving}
                className="mt-3 rounded-lg border border-rose-200/35 bg-rose-300/20 px-3 py-1.5 text-xs text-rose-100 transition hover:bg-rose-300/30 disabled:cursor-not-allowed disabled:opacity-60"
              >
        ...
        <article className="rounded-2xl border border-white/10 bg-black/25 p-4">
          ...
          <div className="mt-3 h-[360px] overflow-auto rounded-xl border border-white/10 bg-black/20 p-3 text-sm text-slate-200">
            {!activeMeta ? (
      ...
      <div className="rounded-2xl border border-white/10 bg-black/25 p-4">
        ...
        <div className="mt-3 space-y-2">
          {!activeMeta ? (
            <p className="text-xs text-slate-500">{t('Not secili degil.', 'No note selected.')}</p>
          ) : versions.length === 0 ? (
            ...
            versions.slice(0, 10).map((version) => (
              <div
                key={version.id}
                className="flex items-center justify-between rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs text-slate-200"
              >
